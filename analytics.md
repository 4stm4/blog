...
Описание событий Umami, которые отправляет блог. Кастомные события идут через `window.blogAnalytics.track(...)`: если `window.umami.track` ещё не доступен, событие временно кладётся в очередь. Пользователи не идентифицируются.

## article_view
- **Когда**: на страницах с макетом `post` сразу после загрузки, с повторными попытками до готовности analytics helper.
...
- **Поля**:
  ...
  - `utm_source`, `utm_medium`, `utm_campaign`, `utm_content`, `utm_term`, `ref`: пробрасываются, если присутствуют в query-параметрах. Эти значения сохраняются в `sessionStorage` и добавляются к последующим кастомным событиям в рамках сессии.
