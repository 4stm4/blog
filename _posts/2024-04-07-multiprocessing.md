...
1. Пример с разделяемым словарем (`dict()`):
...
1. Пример с разделяемой переменной (`Value()`):
...
1. Пример с разделяемым массивом (`Array()`):
...
```
 ...
 1. Пример использования блокировки (`Lock()`):
